...
否则中文会转义为Unicode编码。
 ...
 crypt() 函数返回使用 DES、Blowfish 或 MD5 算法加密的字符串。它的语法是crypt(str,salt)，在这里，str是必填的字段，一般为字符串。salt是可选字段。用于增加被编码字符数目的字符串，以使编码更加安全。需要注意的是，如果未提供salt的参数，则每次调用这个函数时会随机生成一个。在进行密码校验，判断登录用户密码是否一致等场合，如果不提供salt参数，则会导致两次得到的结果不一致！这点需要特别注意！
  ...
  如果把该方法的参数设置为 true，那么无论文档的最后修改日期是什么，它都会绕过缓存，从服务器上重新下载该文档。这与用户在单击浏览器的刷新按钮时按住 Shift 健的效果是完全一样。  
   ...
   今天同事codereview我的代码，被他指出了很多错误！其中大部分都是细节问题。他说的最多的就是，你要注意啊，这些都是细节问题！。。。真不知道该说自己什么好
，每次写完代码，你认真回头看过没？检查过没？有没有想过更好的解决方式？。。。这些都是自己身上存在的问题！特别是细节问题！小伙子，你要加油了！


   # 2017-12-12

   php中，三元运算符 ? : 的使用：$flag  ? $flag :$other  可以简写为   
   $flag ?:$other.


   # 2018-03-02

   在使用strpos()函数时，查找字符串首次出现的位置，判断函数的返回值是否是整数时，应该使用"==="，而不能使用"==",因为这个函数可能返回布尔值 FALSE，但也可能返回等同于 FALSE 的非布尔值。例如，查找字符出现在第一个位置时，它返回的是0。类似的函数还有stripos(), strrpos(),strripos()。